...
export class CustomSocket {
  ...
  private setupChatSocket() {
    this._io.of("chat").on("connection", async (socket: Socket) => {
      const userId = socket.handshake.query.userId as string;
      this._logger.logInfo("User connected", { userId });
  ...
  private setupSupportChatSocket() {
    this._io.of("support").on("connection", async (socket: Socket) => {
      const userId = socket.handshake.query.userId as string;
      this._logger.logInfo("Support connected", { userId });
      await this.updateUserOnlineStatus(socket, userId, true);
      await this.handleConnection(socket);
    });
  }
  ...
  private async handleJoin(socket: Socket, chatId: string) {
    try {
      this._logger.logInfo("User joined chat", { chat_id: chatId });
      socket.join(chatId);
    } catch (error) {
  ...
  private async handleMessage(socket: Socket, message: Message) {
    try {
      this._logger.logInfo("Message received", message);
  ...
  private async handleChatsList(socket: Socket, userId: string) {
    try {
      this._logger.logInfo("Getting chats for user", { userId });
