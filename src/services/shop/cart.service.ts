...
class CartService {
    ...
    async getCartItems(userId: string): Promise<ICart | ApiError> {
        try {
            ...
            await this.logger.logInfo(`Retrieved cart items successfully for User: ${userId}`)
            return cartItems;
        } catch (error: any) {
            await this.logger.logError(`Error while retrieving cart items for User: ${userId}. Error: ${error.message}`);
    ...
    async addCartItem(userId: string, productId: string, quantity: number): Promise<ICart | ApiError> {
        try {
            ...
            if (!product || product.quantity < quantity) {
                await this.logger.logError(`Product not found or insufficient quantity for productId: ${productId}`);
            ...
            if (!cart) {
                ...
                await this.logger.logInfo(`Product added to the cart. ProductId: ${productId}, Quantity: ${quantity}`);
            }
            return cart;
        } catch (error: any) {
            await this.logger.logError(`Error while adding cart item for User: ${userId}. Error: ${error.message}`);
            return ApiError.BadRequest(error.message);
        }
    }
    ...
    async removeItem(userId: string, productId: string): Promise<ICart | ApiError> {
        try {
            ...
            await this.logger.logInfo(`Product removed successfully from User: ${userId}'s cart`);
            return cart;
        } catch (error: any) {
            await this.logger.logError(`Error while removing product from User: ${userId}'s cart. Error: ${error.message}`);
            return ApiError.BadRequest(error.message);
        }
    }
}
